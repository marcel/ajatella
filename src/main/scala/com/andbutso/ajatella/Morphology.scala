...
object Morphology {
  ...
  val PossibleStructures = Map(
    ...
    206 -> Morphology("N", Letters(V), None, None, None, Some(Left(Case.Elative.stA)), Some(Possessive.Two))
  )
}
...
case class Morphology(
  ...
  stemEnding: Letters, // V/C, V, VV, etc
  ...
  caseOrPerson: Option[Either[Letters, Person]], //
  ...
  def matches(lexeme: Lexeme) = {
    ...
    caseOrPerson flatMap {
      case Left(suffix) if lexeme.endsWith(suffix.toString(lexeme)) =>
        lexeme.drop(suffix.toString(lexeme))
      case _ => None
    } map { truncated =>
